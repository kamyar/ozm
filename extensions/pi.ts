...
function ozmEnv(env: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  return {
    ...env,
    OZM_AGENT_NAME: AGENT_NAME,
    OZM_AGENT_DESCRIPTION: env.OZM_AGENT_DESCRIPTION ?? AGENT_DESCRIPTION,
    OZM_SAFE_READONLY: "1",
  };
}

function ozmCommandHelp(command: string): string | undefined {
  try {
    return execFileSync("ozm", [command, "--help"], {
      encoding: "utf8",
      env: ozmEnv(process.env),
...
const OZM_BASH_HELP = ozmCommandHelp("bash") ?? "";
const OZM_TRUST_HELP = ozmCommandHelp("trust") ?? "";
const OZM_HAS_BASH = OZM_BASH_HELP.includes("--command") && OZM_BASH_HELP.includes("--title");
const OZM_HAS_TRUST_JSON = OZM_TRUST_HELP.includes("--check") && OZM_TRUST_HELP.includes("--json");
...
function stableTitle(prefix: string, command: string, cwd: string): string {
...
function writeReviewedScript(command: string, cwd: string): string {
  ...
  const path = join(OZM_SCRIPT_DIR, `${stableTitle("pi-bash", command, cwd)}.sh`);
...
function previewThenRunBash(command: string, cwd: string): string {
  const title = stableTitle("pi-bash", command, cwd);
  const script = `#!/usr/bin/env bash\n# Reviewed by ozm via pi bash integration.\n${command}\n`;
  const previewStart = `----- BEGIN ozm shell review: ${title} -----`;
  const previewEnd = "----- END ozm shell review -----";
  return [
    `printf '%s\\n' ${shellQuote(previewStart)}`,
    `printf '%s' ${shellQuote(script)}`,
    `printf '%s\\n' ${shellQuote(previewEnd)}`,
    `ozm bash --title ${shellQuote(title)} --command ${shellQuote(command)} -- ${ozmMetadata(SCRIPT_DESCRIPTION)}`,
  ].join("; ");
}
...
function routeThroughOzm(command: string, cwd: string): string {
  ...
  const needsShellSyntax = hasShellMetacharacters(trimmed) || !words;
  const needsShell = Boolean(words && hasLeadingShellEnvAssignment(words));

  if (!needsShellSyntax && !needsShell && name === "ozm") {
    return trimmed;
  }

  if (!needsShellSyntax && !needsShell && startIndex === 0 && name && SAFE_DIRECT_COMMANDS.has(name)) {
    return trimmed;
  }

  if (!needsShellSyntax && !needsShell && name === "git" && startIndex !== undefined) {
    const rest = quotedWords(words!.slice(startIndex + 1));
    return `ozm git ${ozmMetadata()} -- ${rest}`.trim();
  }

  if (!needsShellSyntax && !needsShell && words && isLikelyScript(words, cwd) && startIndex !== undefined) {
    const scriptPath = resolve(cwd, words[startIndex]);
    return previewThenRunScript(scriptPath, quotedWords(words.slice(startIndex)));
  }

  if (!needsShellSyntax && !needsShell && words) {
    return `ozm cmd ${ozmMetadata()} -- ${quotedWords(words)}`;
  }

  if (OZM_HAS_BASH) {
    return previewThenRunBash(trimmed, cwd);
  }
...
type TrustCheck = {
  repo_config_exists?: boolean;
  trusted_config_exists?: boolean;
  trusted_config_differs?: boolean | null;
};

function parseJsonObject<T>(output: string): T | undefined {
  try {
...
async function getTrustCheck(ctx: ExtensionContext): Promise<TrustCheck | undefined> {
  if (OZM_HAS_TRUST_JSON) {
    const trust = await runOzm(["trust", "--check", "--json"], ctx);
    return trust.ok ? parseJsonObject<TrustCheck>(trust.output) : undefined;
  }
  ...
  return {
    repo_config_exists: existsSync(resolve(ctx.cwd, ".ozm.yaml")),
    trusted_config_exists: trusted,
  };
}
...
async function bootstrapCheck(ctx: ExtensionContext, notifyWhenHealthy = false) {
  ...
  const trustStatus = await getTrustCheck(ctx);
  const hasRepoConfig = trustStatus?.repo_config_exists ?? existsSync(resolve(ctx.cwd, ".ozm.yaml"));
  if (hasRepoConfig && trustStatus?.trusted_config_exists === false) {
...
export default function (pi: ExtensionAPI) {
  ...
  const bashTool = createBashTool(cwd, {
    ...
    spawnHook: ({ command, cwd, env }) => ({
      ...
      cwd,
      env: { ...ozmEnv(env), PI_OZM_INTEGRATION: "1" },
    }),
  });
  ...
  pi.on("before_agent_start", async (event) => ({
    systemPrompt: `${event.systemPrompt}\n\nOzm integration is active for the bash tool. Commands are routed through ozm cmd/git/run/bash with pi-provided metadata. If ozm asks for changes, follow the ozm feedback exactly.`,
  }));
  ...
  pi.registerCommand("ozm-bootstrap", {
    ...
    handler: async (_args, ctx) => {
      ...
      if (trustStatus?.trusted_config_exists === false) {
