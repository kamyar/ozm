...
Scripts must have a shebang, but the source file does not need an executable file mode. Do not run `chmod +x` before `ozm run`; ozm executes the reviewed content from a private, user-executable snapshot. Ozm rejects scripts with only one executable line before cache or approval checks. It also rejects disk, stdin, and generated in-memory shell files when every command segment is an `ozm` invocation. Run those Ozm commands directly and one at a time so normal automatic approvals can apply. Run other single commands directly with `ozm cmd`, `ozm gh`, or `ozm git`; use `ozm bash --command` only when shell syntax is required. Generated one-line commands without shell operators, substitutions, expansions, or redirections are redirected to their direct Ozm family before approval. The first approval stores the script's SHA-256 hash under the current project. Unchanged scripts run without prompting. Changed persistent files show a diff against their last approved snapshot. Generated stdin and shell content always shows a complete review because reused generated titles do not prove that two snippets are related. Shell snippets use command presentation instead of file presentation.
...
Conservative built-in reads such as `command -v`, `bazel query`, `brew search`, `npm view`, and `npm list` run without command approval or cache checks. Exact metadata, help, and disk-usage forms for trusted Pi, Bazel, Go, Docker, OrbStack, and system tools also run as semantic reads from trusted executable paths. Write-capable and unknown forms retain normal policy. `ozm cmd` executes argv directly rather than through a shell. It detects direct, interpreter, and wrapper-based script execution, preserves script arguments in the `ozm run` nudge, applies enabled private entry-point redirects before policy or cache checks, refuses `ozm cmd git ...` in favor of `ozm git`, refuses direct `gh` or `ozm cmd gh ...` usage in favor of `ozm gh`, and hard-blocks cases that are unsafe to blanket approve, including `sed`, `gsed`, and `rg --pre`. When `chmod` targets a file modified in the last 10 minutes, Ozm requires `--confirm-recent-chmod` before normal command policy checks continue. Bare `env` output is blocked because it can expose secrets. Executables under temporary locations bypass config and cache; they require a reasoned one-time override, display their SHA-256 digest, and are rechecked after approval.

Use `ozm gh` for GitHub operations. It applies the normal blocklist, allowlist, cache, and approval flow, then resolves `gh` from trusted system locations. Use typed `ozm gh pr review-reply` and `ozm gh issue add-sub-issue` operations for fixed repository writes. Use `ozm gh issue create-batch --repo OWNER/REPOSITORY --manifest FILE` to review multiple issue creations once without adding persistent write authorization; Ozm rejects equivalent raw REST POST requests and prints typed replacements. Exact repositories can authorize these operations under `github.allowed_operations`. Known high-level reads and unambiguous REST `GET` and `HEAD` requests can run without an approval dialog. If `-f` or `-F` would make a proven GET-only REST endpoint default to POST, Ozm stops before approval and prints an explicit `--method GET` replacement. REST writes, ambiguous methods, file-backed input, unsafe method-override headers, and absolute non-GitHub endpoints still require review. GraphQL requests such as `gh api graphql -f query=...` are auto-allowed only when the selected operation is definitely a query. Mutations, file-backed queries, malformed documents, or ambiguous multi-operation requests still require review.
...
All git subcommands pass through, but policy checks apply to commits, pushes, dangerous history-rewrite commands, and dangerous config keys. Commit messages must use one single-line `-m "message"` with a 72-character subject limit and 500-character total limit. Commit-message shape errors cannot be overridden. Broad force pushes are blocked without a dialog. Only full `--force-with-lease=REF:EXPECTED_SHA` options can request a one-time override. An atomic multi-ref rewrite can supply one exact lease for each forced ref. Pushes to `main` or `master` remain override-gated.
...
### Private tools and policies

Keep employer-private tools, repository names, endpoints, symbols, and policies out of Ozm's public source, tests, examples, and generated instruction templates. Put private command routing in data-only rule packs under `~/.ozm/rule-packs/`. Enable packs explicitly from user-owned Ozm configuration. Keep repository-specific authorization in `~/.ozm/config.yaml` or `~/.ozm/projects/`.

Ozm does not load rule packs from a repository. A malformed or missing enabled pack stops the command. Rule-pack redirects only block wrappers and suggest a configured installed entry point. They do not authorize the target command.
