...
class GitHubRESTParserTests(unittest.TestCase):
    def test_implicit_get_with_output_and_pagination_options(self):
        args = [
            "gh", "api", "--paginate",
            "repos/example/widgets/pulls/123/comments",
            "--jq", ".[] | .id",
        ]
        ...
        self.assertEqual(
            ...
            github_api.GitHubRESTRequest(
                method="GET",
                endpoint="repos/example/widgets/pulls/123/comments",
            ),
        )
    ...
    def test_explicit_get_can_use_non_file_query_fields(self):
        args = [
            "gh", "api", "repos/example/widgets/issues",
    ...
    def test_unsafe_or_ambiguous_requests_are_not_auto_allowed(self):
        cases = [
            ["gh", "api", "-X", "POST", "repos/example/widgets/issues"],
            ["gh", "api", "-X", "DELETE", "repos/example/widgets/issues/123"],
...
class GitHubHighLevelReadTests(unittest.TestCase):
    def test_known_high_level_reads_are_auto_allowed(self):
        cases = [
            ...
            (["gh", "repo", "view", "example/widgets"], "github repo view"),
            (["gh", "search", "code", "WidgetParser"], "github search code"),
...
class GitHubRESTReadAutoAllowTests(unittest.TestCase):
    ...
    def test_rest_get_runs_without_approval_or_cache(self):
        args = [
            "gh", "api", "repos/example/widgets/pulls/123/reviews",
    ...
    def test_supported_rest_write_is_blocked_with_typed_nudge(self):
        args = [
            "gh", "api", "-X", "POST",
            "repos/example/widgets/pulls/123/comments/456/replies",
            "-f", "body=hello",
        ]
